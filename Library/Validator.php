...
class Validator {
     ...
     * @param string $rule
     *            规则名称
     * @param array $exception=array('message'=>'错误信息',
     *            'code'=>'错误码')
     * @return $this
     */
    ...
    public function run (array $data, $rules = NULL) {
        ...
        foreach ($rules as $key => $rule) {
            $this->_key = $key;
            $data[$key] = (is_array($data[$key]) ? 0 == count($data[$key]) : 0 ==
                     strlen($data[$key])) ? NULL : $data[$key];
            foreach ($rule as $k => $v) {
                /**
                 * 获取校验方法名
                 */
    ...
    public function confirm ($str, $key) {
        return ! empty($this->_data[$key]) ? ($str == $this->_data[$key]) : empty(
                $str);
    }
    ...
    public static function url ($str) {
        ...
        return isset($parts['scheme']) && in_array($parts['scheme'], 
                array(
                        'http',
                        'https',
                        'ftp'
                )) && ! preg_match(
                '/(\(|\)|\\\|"|<|>|[\x00-\x08]|[\x0b-\x0c]|[\x0e-\x19])/', $str);
    }
        ...
        for ($i = 0; $i < strlen($search); $i ++) {
            ...
            $str = preg_replace(
                    ...
                    $search[$i], $str); // with
                                        ...
                                        // seven
                                        // times
            $str = preg_replace('/(&#0{0,8}' . ord($search[$i]) . ';?)/', 
                    $search[$i], $str); // with
                                            // a
                                            // ;
        }
